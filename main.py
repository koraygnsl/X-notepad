...
winbox.config(bg = "#0d09ea")
top = tk.Frame(winbox)
top.pack(padx = 10, pady = 5, anchor = 'nw')

buttonOne = tk.Button(winbox, text="Open", bg="#ea09c5", command= openFile)
buttonOne.pack(in_ = top, side =LEFT )

buttonTwo = tk.Button(winbox, text="Save", bg="#ea09c5", command= saveFile)
buttonTwo.pack(in_ = top, side =LEFT)

buttonThree = tk.Button(winbox, text="Clear", bg="#ea09c5", command= clearFile)
buttonThree.pack(in_ = top, side =LEFT)

buttonFour = tk.Button(winbox, text="Exit", bg="#ea09c5", command= exit)
buttonFour.pack(in_ = top, side =LEFT)

entry = tk.Text(winbox, wrap = WORD, bg="#e89a09", font = ("arial", 15))
